...
Web 会在进入选品页时并发读取四个活动页当前品类，按来源分组并默认全选。用户可只保留本次需要的品类；未选择的来源不会启动抓取浏览器，其他来源也只点击被勾选的 Tab。品类列表缓存 30 分钟，可在页面手动刷新；黑色星期五当前作为单一“全场精选（页面无类目Tab）”选项。

“候选池”会保留每个商品的候选排名、规则参考分、AI 是否入选、AI 排名、淘汰理由和合格不足说明，方便追溯模型决策。“选品明细”和“推荐结果”只展示最终入选商品。

## 推荐模式

业务安装包默认通过 AllSpark 模型代理调用 AI；代理未启用、网络不可达或模型调用失败时使用 `explainable_scoring`：按可用字段动态归一化销量、折扣、类目内相对价格、页面/榜单位次、好评率，规则选取最多 10 个作为明确标记的回退结果；缺失字段不会被当作 0 分惩罚。
...
业务安装包默认配置如下，均可使用环境变量覆盖：

```bash
export LIVE_LLM_PROXY_URL='http://114.67.72.156/AllSpark/api/live-tools/llm/chat/completions'
export LIVE_LLM_PROXY_TOKEN='live-tools-llm-2026'
export LIVE_LLM_PROXY_MODEL='OxygenLLM-30B-A3B'
```

这里的 Token 只能访问 AllSpark 的固定模型代理接口，不是 JD 模型网关 Key。真实网关地址、Key 和强制模型保存在 AllSpark 的 Nacos 中。
当前默认地址沿用已有日志服务器的 HTTP 入口，仅适合联调；正式发给业务人员前应给 AllSpark 配置 HTTPS，并通过 `LIVE_LLM_PROXY_URL` 切换到 HTTPS 地址，避免候选商品和代理 Token 在传输链路中明文暴露。

开发者本地也可以复制 `model-config.example.json` 为 `model-config.local.json` 直接连接其他兼容网关。实际配置文件已被 Git 忽略，`SELECTION_AI_*` 环境变量仍具有最高优先级：
...
`model-config.local.json` 只用于开发者本机调试，不能提交 Git 或放入给业务人员的安装包。Windows 与 macOS GitHub Actions 只复制无密钥示例文件，并在发布目录中发现真实本地配置时直接终止构建。不要通过 GitHub Secrets 把模型供应商密钥注入客户端包，因为已下发到用户电脑的密钥仍可被提取。业务包只携带可撤销、仅限固定接口的 AllSpark 访问 Token；供应商 Key 只保存在 AllSpark/Nacos。
...
模型请求使用 SSE 流式接收。考虑 AllSpark 排队和上游生成时间，单次网络读取默认超时 210 秒，可通过 `SELECTION_AI_TIMEOUT_SECONDS` 调整；类目并发数和每秒请求上限可通过 `SELECTION_AI_CATEGORY_WORKERS`、`SELECTION_AI_RPS_LIMIT` 调整。本地最多 5 RPS，AllSpark 再对所有用户执行统一 5 RPS 限流。只有连续 3 个类目发生网络故障才熔断，可通过 `SELECTION_AI_CIRCUIT_FAILURE_THRESHOLD` 调整；业务性少选和 JSON 内容错误不会误触发网络熔断。
