...
def run(headless: bool = False, offline_path: str | None = None, allow_partial: bool = False) -> dict:
    if offline_path:
    ...
    print(f"[main] 解析有效商品 {len(items)} 个")
    candidate_pool = build_candidate_pool(items)
    for source_name, categories in candidate_pool.items():
        for category_name, products in categories.items():
            print(f"[candidate] {source_name}/{category_name}: 候选 {len(products)} 个")
    recommendation = recommend(candidate_pool)
    ...
    for source_name, categories in selection.items():
        for category_name, products in categories.items():
            print(f"[select] {source_name}/{category_name}: 最终入选 {len(products)} 个")
...
def main() -> None:
    ...
    payload = run(args.headless, args.offline, args.allow_partial)

    json_path = save_json(payload, config.OUTPUT_DIR, timestamp)
    excel_path = save_excel(payload, config.OUTPUT_DIR, timestamp)
    print(f"[main] JSON: {json_path}")
    print(f"[main] Excel: {excel_path}")
    print(f"[main] 推荐模式: {payload['recommendation_mode']}")
