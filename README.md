...
6. 页面右上角固定显示当前版本；点击版本入口可以检查更新，发现新版本后入口会持续提示。
7. 成功获取的更新信息会在本地缓存 7 天；刷新、重启或内部更新源临时超时时不会丢失已发现的版本。
8. 内部更新源检查失败时每 60 秒重试；已是最新版时每 30 分钟后台复查一次。

自动更新会保留 `runtime/`、`logs/`、京东登录态和本地模型配置。执行安装前仍建议等待当前测价、截图或自动创建任务结束。

Windows 打包版默认从 `http://stock.seifallspark.com/temp/aiTools/live-tools-update.json` 检查更新。发版后由可访问公司 SFTP 的电脑按以下顺序同步文件，必须最后上传更新清单：

```bash
release_version=0.5.4
scp -P 8922 Live-Tools-Windows.zip sftp@101.124.5.212:/home/allSpark/files/temp/aiTools/Live-Tools-Windows-v${release_version}.zip
scp -P 8922 live-tools-update-internal.json sftp@101.124.5.212:/home/allSpark/files/temp/aiTools/live-tools-update.json
```

上传后应确认安装包支持 Range 请求并校验其 SHA256 与内部更新清单一致。SFTP 密码不得写入仓库或脚本。
...
客户端监控版本采用 SemVer，不带 `v` 前缀；GitHub Release 标签使用对应的 `v` 前缀。本轮客户端统一上报 `0.5.4`，打包发布时使用标签 `v0.5.4`。新增兼容功能递增次版本，问题修复递增补丁版本。
