...
1. **下载 Release**
   ...
   - 下载 `Live-Tools-Windows.zip`

2. **解压运行**
   ```text
   解压 ZIP → 双击根目录的 "启动直播工具.bat"
   ```
...
请打开程序目录下的 `logs/web.log` 查看启动日志。源码模式的 `start.bat` / `start.sh` 会自动检查并安装 Python 依赖和 Playwright Chromium；如果网络限制导致安装失败，日志里会显示具体错误。
</details>
...
| **启动命令** | `python app.py` 或双击根目录 `启动直播工具.bat` | `python main.py` 或 `start.bat` / `start.sh` |
