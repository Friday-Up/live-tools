...
class WebTemplateTest(unittest.TestCase):
    def test_index_contains_product_selection_panel(self):
        ...
        self.assertIn("京东多来源选品", html)
        self.assertNotIn("京东四来源选品", html)
        self.assertIn('id="productSelectionShowBrowser"', html)
        self.assertIn('id="productSelectionCategoryGroups"', html)
        self.assertIn('id="productSelectionCategoryStatus"', html)
        self.assertIn('id="productSelectionSelectAllCategories"', html)
        self.assertIn('id="productSelectionClearCategories"', html)
        self.assertIn('id="productSelectionRefreshCategories"', html)
        self.assertIn("/api/product-selection/categories", html)
        self.assertIn("selected_categories: collectProductSelectionCategories()", html)
        self.assertIn("至少选择一个品类", html)
        ...
        self.assertIn(".loading-dots::after", html)
        self.assertIn("@keyframes loading-dots", html)
        self.assertIn("element.classList.toggle('loading-dots', stage === 'running')", html)
        self.assertIn("#productSelectionStage", html)
        self.assertIn("width: 6em", html)
        self.assertIn("if (productSelectionStartBtn.disabled) return;", html)
        self.assertIn("productSelectionStartBtn.textContent = '选品进行中...'", html)
    ...
    def test_index_contains_price_audit_and_promotion_binding_tabs(self):
        ...
        self.assertIn("使用文档 ↗", html)
        self.assertIn("https://joyspace.jd.com/pages/Knx10idaiNsUUaa7fD0C", html)
        self.assertIn("问题反馈 ↗", html)
        self.assertIn("https://joyspace.jd.com/share/bdBb9YpwG8md7rx86lOW?sharedId=F0GdxTbgTp5KZB&amp;viewType=4", html)
        self.assertIn('class="header-actions"', html)
        self.assertIn('target="_blank"', html)
        self.assertIn('rel="noopener noreferrer"', html)
        self.assertIn('id="updateNotice"', html)
        self.assertIn('id="updateCheckButton"', html)
        self.assertLess(html.index("使用文档 ↗"), html.index('id="updateCheckButton"'))
        self.assertLess(html.index("问题反馈 ↗"), html.index('id="updateCheckButton"'))
        self.assertIn("`版本 v${state.current_version}`", html)
        self.assertIn("`● 新版本 v${state.latest_version}`", html)
        self.assertIn("`安装 v${state.latest_version}`", html)
        self.assertIn("当前平台暂不支持自动更新", html)
        self.assertIn(".update-release-link[hidden]", html)
        self.assertIn('id="updateActionButton"', html)
        self.assertIn("/api/update/check", html)
        self.assertIn("/api/update/download", html)
        self.assertIn("/api/update/install", html)
        self.assertIn("正在后台下载", html)
        self.assertIn("scheduleUpdateRecheck(", html)
        self.assertIn("60 * 1000", html)
        self.assertIn("30 * 60 * 1000", html)
        self.assertIn("查看版本与更新", html)
        self.assertIn("manifest-cache.json", (Path(__file__).parents[1] / "update_manager.py").read_text(encoding="utf-8"))
        self.assertIn("SKU 测价、绑定券码/促销ID、批量创建直播间、蓝屏自动截图、自动创建红包雨、选品 Agent", html)
        ...
        self.assertIn("/api/price-audit/template", html)
        self.assertNotIn("/api/promotion-binding/input-template", html)
        self.assertIn("下载简易模板（可选）", html)
        self.assertIn("其他列可保留", html)
    ...
    def test_index_contains_room_creator_panel(self):
        ...
        self.assertIn("/api/room-creator/template", html)

    def test_index_contains_red_rain_creator_panel(self):
        app = create_app(base_dir=Path(tempfile.mkdtemp()))
        response = app.test_client().get("/")

        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn('data-tool="redRainPanel"', html)
        self.assertIn('id="redRainPanel"', html)
        self.assertIn("/api/red-rain/template", html)
        self.assertIn("/api/red-rain/preview", html)
        self.assertIn("/api/red-rain/start", html)
        self.assertIn("/api/red-rain/status", html)
        self.assertIn("/api/red-rain/download", html)
        self.assertIn('id="redRainStatPending"', html)
        self.assertIn('id="redRainResultPending"', html)
        self.assertIn("redRainTaskRunning", html)
        self.assertIn('id="redRainActions"', html)
        self.assertIn("固定使用 CMC 红包创建", html)
        self.assertNotIn("普通发放需填写中奖概率", html)
        self.assertIn('font-size: 14px; color: #666; line-height: 1.8; margin-bottom: 16px;', html)
        self.assertIn('id="showRedRainBrowserInput"', html)
